...
- [ ] 프론트엔드 데이터 패칭 레이어 개선 (Nuxt 기본 fetcher 전환)
  - [x] `useGet`, `usePost` 등 커스텀 컴포저블 `useFetch` / `$fetch` 기반 전환
  - [x] Vue Query 플러그인 및 관련 패키지 제거
  - [ ] 전역 쿼리 키 팩토리 유지 여부 검토
  - [ ] GET 수동 실행 규칙(`execute` / `refetch`) 호출부 정비
