export default defineEventHandler(async (event) => {
  ...
  // 기본 정보
  // ========== ========== ========== ==========
  const query = getQuery<PlayerQueryDto>(event);
  ...
  // 1. Where 조건 구성
  const userColumns = getTableColumns(playersTable);
  const where = buildDrizzleWhere<PlayerQueryDto>(query, {
    id: 'eq',
    idList: 'in',
    name: 'like',
    role: 'eq',
  ...
  // 5. 결과 반환
  const listData = new ListData<PlayerOutDto>(
    users as PlayerOutDto[],
    totalElements,
    filteredElements,
    isPaged
      ? page
