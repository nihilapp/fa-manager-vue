export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<CommonInDto & { status: Status }>(event);
  ...
  const message = RESPONSE_MESSAGE.CAMPAIGN_STATUS_UPDATED(
    String(id),
    oldStatus,
    body.status
  );
  ...
  // 단건이면
  return BaseResponse.data(result[0] as CampaignOutDto, RESPONSE_CODE.OK, message);
