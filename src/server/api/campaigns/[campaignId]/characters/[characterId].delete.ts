...
export default defineEventHandler(async (event) => {
  ...
  const result = await db.update(charactersTable).set({
    ...
    and(
      ...
      eq(charactersTable.campaignId, Number(campaignId))
    )
  ).returning();
  ...
  // 단건이면
  return BaseResponse.data(result[0] as CharacterOutDto, RESPONSE_CODE.OK, RESPONSE_MESSAGE.CHARACTER_DELETED);
