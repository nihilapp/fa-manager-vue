...
export default defineEventHandler(async (event) => {
  ...
  const classNameParam = decodeURIComponent(getRouterParam(event, 'className') || '');
  const body = await readBody<CharacterClassUpdateDto>(event);

  if (!Number.isFinite(characterId) || !classNameParam || !body) {
  ...
  const existingClass = await db.query.characterClassesTable.findFirst({
    where: (table, { eq, and, }) => and(
      ...
      eq(table.className, classNameParam)
    ),
  });

  if (!existingClass) {
    return BaseApiResponse.error(RESPONSE_CODE.NOT_FOUND, RESPONSE_MESSAGE.CHARACTER_CLASS_NOT_FOUND);
  }

  await db.update(characterClassesTable).set({
    subClassName: body.subClassName?.trim() ?? existingClass.subClassName,
    level: body.level ?? existingClass.level,
  }).where(
    and(
      eq(characterClassesTable.characterId, characterId),
      eq(characterClassesTable.className, classNameParam)
    )
  );
  ...
  return BaseApiResponse.data(
    ...
    RESPONSE_CODE.OK,
    RESPONSE_MESSAGE.UPDATE_CHARACTER_CLASS_SUCCESS
  );
});
